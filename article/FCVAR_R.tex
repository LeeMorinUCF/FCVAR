...
In addition, the user can test any linear restriction % on the cointegrating relations, the adjustment coefficients, or 
...
R> opt$db0 <- c(0.80, 0.80) # Set starting values for optimization.
R> opt$gridSearch <- 0   # Use given starting values instead of a grid search.
\end{Code}
...
the identification problem is alleviated if the estimator of the pair of parameters $[d, b]$ is chosen 
...
% Maybe a subfig vs subfigure difference. 
% At least it works.
\begin{figure}[H]
  \centering
