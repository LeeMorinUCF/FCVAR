...
The \pkg{PACKAGE-NAME} provides a careful optimization procedure by showing estimates corresponding to multiple local optima---a common occurrence in these models---and this feature is accounted for in \pkg{FCVAR} as well.
% 
...
% 
One feature that is missing, however, is the maximum likelihood method of \citet{Johansen1995}, which he extended to fractional processes in \cite{Johansen2008}. % (\textbf{cite JN papers}).
This framework allows for joint testing of a comprehensive set of restrictions and provides the 
foundation on which the \pkg{FCVAR} package is built. 
...
The next section describes the FCVAR model and the restricted models that can be estimated with this program. Section \ref{sec:main} describes an example of a modeling session, which is a replication of one of the tables of results in \cite{JNP2014}. Section \ref{sec:examples} describes another example program, which demonstrates some additional functionality of the software. 

% Importantly, these are the only two files that would need to be changed to apply the program for other empirical analyses. 
...
which is the reason the \textbf{package\_name} package, designed for univariate series, 
employs a series of optimization routines, each with different starting values, 
...
It also allows for the possibility of overcoming an identification problem outlined in \textbf{[Reference goes here]}. 
When the likelihood function has multiple local optima, 
the identification problem is overcome if the estimator of the pair of parameters $[d, b]$ is chosen 
with the highest value of $b$, among the local optima. 
...
is endowed with the \fct{plot} method. 
...
According to the recommendation of \textbf{[Reference goes here]}, 
...
the user can be assured that the optimization of the likelihood is functioning properly.  
...
% Maybea subfig vs subfigure difference. 
% At least it works.
\begin{figure}[H]
  \centering
...
The \pkg{FCVAR} package is available on 
the Comprehensive
...
A development version is stored in a GitHub repository and can be installed by entering 
...
\end{Code}
 ...
 Under this approach, users must have installed the \pkg{devtools} package 
and have the supplementary toolkit \pkg{Rtools} installed. 
