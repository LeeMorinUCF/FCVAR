...
\Abstract{
  ...
  Existing software concentrates on either the CVAR alone, which is restricted to integer orders of integration, or 
  on specific features of the variables, such as estimating the integration order, identifying whether a  cointegrating relationship exists, bivariate anaysis or the focus on a particular response variable.
  This software treats the time series as a sytem and estimates all parameters together in a likelihood ratio framework and it provides a flexible set of options for conducting inference on many features of the cointegrating relationship. 
...
Following this approach, the time series are treated as an endogenous system of equations and permits the estimation of a higher-dimensional cointegrating relationship, i.e. several equilibrium relationships. 
% 
...
The \pkg{urca} package 
\citep{urca2016}\footnote{They also use the Danish data, so it might be worthwhile to include in the documentation an example with and without the restriction $d = b = 1$ to compare}, 
...
While the packages that follow the framework of \citet{Johansen1995} are most closly related to the \pkg{FCVAR} package, these are not suited to the analysis of series with a fractional degree of integration. 
...
The package \pkg{LongMemoryTS} is in a class of its own, in that it uses a wide variety of methods methods to investigate both cointegrating relationships and fractional integration.\footnote{Morten: You would be best suited to comment on the references in this package. 
There are several citations to your papers and I want to make sure that we are honest about the difference between what we do and what they do. 
It seems to me that this package (\cite{LongMemoryTS2019}) chains together several separate issues, such as, estimating the fractional integration parameter and then plugging that value into the rank test as if it were known. 
Another example is testing cointegration by estimating the fractional parameter from the residuals. 
It seems to me that this is a who's who of analyzing the cointegration of fractional systems, except for Johansen's MLE framework, which is what we implement.
Our approach following \citet{Johansen1995} is more holistic, in that all the parameters are estimated jointly, aside from the rank and lag selection. 
It captures all of these in one maximum likelihood framework and approach has the added benefit of allowing for a wide range of restrictions to test} 
...
%Ours is more (what's the word?) holistic, in that all the parameters are estimated jointly, aside from the rank and lag selection. It captures all of these in one maximum likelihood framework. 
%This approach has the added benefit of allowing for a wide range of restrictions to test. 


The \pkg{FCVAR} package, introduced here, is closest to a cross between the Johansen cointegration model in \pkg{urca} and the models involving fractionally integrated variables discussed above. 
In particular\footnote{Maybe we should add this sort of example to the documentation, since we had something similar in the first version.}, the model estimated in the \pkg{urca} packages is the special case of \pkg{FCVAR} in which the fractional integration parameters $d$ and $b$ are both equal to one. 
...
The $p$~values for rank tests are obtained using the 
