...
Stolen from \emph{CRAN Task View: Time Series Analysis} by Rob J Hyndman 
\verb|https://CRAN.R-project.org/view=TimeSeries|:
...
\footnote{Note that the \fct{diffseries} function in \pkg{fracdiff} is based on the same algorithm in \cite{Jensen2014} as \fct{FracDiff} in \pkg{FCVAR}, except that \fct{diffseries} demeans the data first. Specifically, \code{all(fracdiff::diffseries(x, d) == FCVAR::FracDiff(x - mean(x), d))} returns \code{TRUE}.}
	\item \pkg{arfima} \cite{arfima2018}
	\item \pkg{nsarfima} \cite{nsarfima2019}
	\item \pkg{LongMemoryTS}: It seems to me that this package (\cite{LongMemoryTS2019}) chains together several separate issues, such as, estimating the fractional integration parameter and then plugging that value into the rank test as if it were known. Another example is testing cointegration by estimating the fractional parameter from the residuals.  
...
This package is closest to a cross between the Johansen cointegration model in \pkg{urca} and the fractionally differencing models in \pkg{a}. 
